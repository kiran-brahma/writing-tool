...
|---|---|
| 100% client-side. No server action, no server storage. | Drafts, revisions, findings and API keys live only in the browser's IndexedDB. |
...
- Diagnosis text is selectable (you may want to keep the finding). The quarantined
  diagnostic-rewrite pane is `user-select: none` and has no insert affordance.
...
- Per-finding `reject` action, plus a global **"show raw provider response"** toggle.
...
pen. There is no length gate and no draft-completeness check — those would be a nanny.
...
Pass {
  ...
  model_slot: "critic",      // override per pass
  enabled, prompt_version
}
```

**Tier 1 — rule-based, local, deterministic, free, offline.** These run automatically on save and
are constitution-safe *by construction*: a regex cannot suggest a word or praise you. Their
configuration (word lists, regexes, windows) is editable data.
...
**Tier 2 — model passes (judgment; regex cannot do these).** Enabled by default: 8, 10, 11, 12, 13.
...
14. Reader reconstruction — see §6
...
- Local passes (all Tier 1, 7, 11, 12, 13) get **target paragraph + one paragraph either side +
  heading outline** (headings only, never body text).
...
| Google Gemini | `https://generativelanguage.googleapis.com/v1beta` | gemini | `x-goog-api-key` | — |
| OpenRouter | `https://openrouter.ai/api/v1` | openai | `Authorization: Bearer` | optional `HTTP-Referer`, `X-OpenRouter-Title` |
| Ollama (local) | `http://localhost:11434/v1` | openai | none | requires `OLLAMA_ORIGINS=https://<app-origin>` |
| Custom | user-supplied | openai | user-supplied | the documented escape hatch for a self-hosted proxy |

**Ollama Cloud is deliberately not a provider.** `ollama.com` returns `405` to CORS preflight with
...
at all. Anyone who wants the hosted endpoint may point a Custom provider at their own proxy.
...
- Runs are on demand, one pass at a time, plus "Run local passes" (free, instant, no key) and
  "Run structural set". Concurrency capped at 3 per provider, configurable, with a visible queue.
...
- "Test connection" on every provider config.
...
that holds the drafts makes it mostly theatre, while "never persisted" is the honest form of *more
secure*. Non-negotiable regardless: keys are never logged, never placed in a URL, never sent to any
origin but the provider's; strict CSP (no inline script, no remote script) set as a Worker header;
`noopener noreferrer` on outbound links; keys excluded from exports by default.

## 6. The Reader

A pass, not a third provider slot. One model call per section returning
`{what_this_section_says, what_a_distracted_reader_would_miss, gap_between_intent_and_effect}`,
shown in its own **Reader** tab so it never mixes with findings. It is the cheapest defence against
...
- **Revisions:** auto-snapshot on debounce (~60s idle or N keystrokes, pruned) **plus** a manual
  "flag major revision" carrying an optional note. Snapshot = full text + metadata (parent id,
  timestamp, word count, flag, findings resolved, active pass run).
...
- **Export:** Markdown always. Single-document JSON bundle (doc + revisions + findings + pass
  results) for round-tripping. Whole-workspace JSON backup, **keys excluded by default** and
...
- **Migrations:** forward-only, versioned. A browser holding a newer schema than the code refuses to
  open and says so, rather than risking drafts.
- **PWA:** manifest + service worker so the shell and existing drafts open offline. No push, no
  background sync.
...
- Drafts, revisions, findings and keys live in this browser's IndexedDB — open DevTools →
...
scope. Run manually before any prompt change; results stored locally with a timestamp. It never
...
- **Stage 0 — Shell (no AI).** Vite/React/TS/Tailwind, Dexie schema, library screen, TipTap editor,
  Markdown import/export, provider config with all five prefills + Custom + Test connection, key
...
- **Stage 2 — Rule engine.** All Tier 1 passes, editable rule config, auto-run on save, metrics panel.
  *Useful with zero API keys.*
- **Stage 3 — Structure.** Section model, outline, document-scope passes, chunking and char limits.
- **Stage 4 — Judge.** Revisions, snapshots, word-level diff, swapped-order double call, `unstable`
  surfacing, screening-frame setting.
- **Stage 5 — Workbench.** Pass editor, rule config editor, critique-prompt authoring assistant.
- **Stage 6 — Reader.** The Reader tab and its schema.
- **Stage 7 — Durability.** Workspace JSON backup, PWA service worker, migrations, backup nag.

## 12. Non-goals

Generation of any prose. Collaboration. Sync. Accounts. Folders. PDF export. Streaming. Prompt-
injection theatre. Clipboard policing. Draft-length gates. Prompt-editable JSON Schema. Any proxy
