 ...
 * last-backed-up reminder remains the real defence — so the returned promise
 * never rejects and a refusal never fails the save that triggered it. This
 * module remembers that it asked, so exactly one request is made per page load
 * no matter how many saves follow.
 */

let requested: Promise<void> | null = null;

/** Ask the browser to persist the Library. Never rejects; safe to ignore. */
export function requestPersistentStorage(): Promise<void> {
  requested ??= persistLibrary();
  return requested;
}
